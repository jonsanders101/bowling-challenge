describe("Game", function () {
  ...
  describe("play", function () {
    ...
    describe("when no argument has been passed", function () {
      it("returns a random bowl that does not exceed remaining pins", function () {
        spyOn(Math, 'random').and.returnValue(0.99)
        game.play(9)
        game.play()
        expect(game.scorecard[1][2]['hitPins']).toEqual(1)
      })
      it("bowls a random number", function () {
        spyOn(game, '_randomBowl')
        game.play()
        expect(game._randomBowl).toHaveBeenCalled()
      })
    })
